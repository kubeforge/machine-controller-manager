...
func validateKubeVirtNetworkInterfaces(nets []*machine.KubeVirtNetworkInterface, fldPath *field.Path) field.ErrorList {
	...
	for i, net := range nets {
		idxPath := fldPath.Index(i)
		if net.NetworkName == "" {
			allErrs = append(allErrs, field.Invalid(idxPath.Child("networkName"), net.NetworkName, "networkName is required for network"))
		}
		if net.NetworkType == "" {
			allErrs = append(allErrs, field.Invalid(idxPath.Child("networkType"), net.NetworkType, "networkType is required for network"))
		}
		if net.NetworkRef == "" {
			allErrs = append(allErrs, field.Invalid(idxPath.Child("networkRef"), net.NetworkType, "networkRef is required for network"))
		}
	}
